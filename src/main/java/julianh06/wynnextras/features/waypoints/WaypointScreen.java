...
public class WaypointScreen extends Screen {
    ...
    protected WaypointScreen() {
        ...
        addNewButton = new EasyButton(-1, -1, -1, -1) {
            ...
            public void click() {
                if(inMainScreen) {
                    ...
                    WaypointData.INSTANCE.waypoints.add(waypoint);
                    ...
                    WaypointData.INSTANCE.categories.add(category);
                }
                WaypointData.save();
            }
        ...
        editCategoriesButton = new EasyButton(-1, -1, -1, -1) {
            ...
            public void click() {
                ...
                activeCategories.addAll(WaypointData.INSTANCE.categories);
                scrollOffset = 0;
                ...
                for (int i = 0; i < Math.min(categories.size(), WaypointData.INSTANCE.categories.size()); i++) {
                    CategoryElement element = categories.get(i);
                    WaypointData.INSTANCE.categories.get(i).name = element.nameInput.getInput();
                    WaypointData.INSTANCE.categories.get(i).color = element.colorPicker.getSelectedColor();
                }
        ...
        categoryDropdown = new EasyDropdown(0, 0, -1, -1)  {
            ...
            public void click() {
                if(!isExpanded) {
                    ...
                    int ySection = Math.floorDiv(mouseY, (39 / scaleFactor));
                    if(ySection < 1) {
                        isExpanded = false;
                        return;
                    }

                    WaypointCategory category = WaypointData.INSTANCE.categories.get(ySection - 1);
            ...
            @Override
            public void draw(DrawContext context) {
                List<WaypointCategory> categoriez = WaypointData.INSTANCE.categories;

                RenderUtils.drawTexturedRect(context.getMatrices(), nameBackgroundTexture, 0, 0, (float) (89 * 3) / scaleFactor, (float) (13 * 3) / scaleFactor, 89 * 3 / scaleFactor, 13 * 3 / scaleFactor);


                FontRenderer.getInstance().renderText(context.getMatrices(), StyledText.fromComponent(Text.of("Category filter")), 8f / scaleFactor, 8f / scaleFactor, CustomColor.fromHexString("FFFFFF"), HorizontalAlignment.LEFT, VerticalAlignment.TOP, TextShadow.NORMAL, 2.75f / scaleFactor);
                ...
                if (isExpanded) {
                    ...
                    for(int i = 0; i < categoriez.size(); i++) {
                        ...
                        if(activeCategories.contains(categoriez.get(i))) {
                            textColor = WaypointData.INSTANCE.categories.get(i).color;
                        }
                        FontRenderer.getInstance().renderText(context.getMatrices(), StyledText.fromComponent(Text.of(WaypointData.INSTANCE.categories.get(i).name)), x + 6f / scaleFactor, y + height + i * 39f / scaleFactor + 10f / scaleFactor, textColor, HorizontalAlignment.LEFT, VerticalAlignment.TOP, TextShadow.NORMAL, 3f / scaleFactor);
                    }
                }
            }
            ...
            public boolean isClickInBounds(int x, int y) {
                ...
                if(isExpanded && WaypointData.INSTANCE.categories != null) {
                    if(y > this.y + height + WaypointData.INSTANCE.categories.size() * (39f / scaleFactor)) return false;
                } else {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if(inMainScreen) {
            for (WaypointElement element : elements) {
                if(element.waypoint.getCategory() != null) {
                    if(!WaypointData.INSTANCE.categories.contains(element.waypoint.getCategory())) {
        ...
        int y = (int) yOffset + (i * 180) / scaleFactor - scrollOffset;
        addNewButton.setX(x);
        addNewButton.setY(y);
    ...
    public void init() {
        ...
        for(WaypointCategory category : WaypointData.INSTANCE.categories) {
            categories.add(new CategoryElement(category));
        }
        ...
        if(inMainScreen) {
            for (WaypointElement element : elements) {
                ...
                if (element.deleteButton.isClickInBounds(mouseX, mouseY)) {
                    ...
                    WaypointData.INSTANCE.waypoints.remove(element.waypoint);
                }
            ...
            for(CategoryElement element : categories) {
                if (element.deleteButton.isClickInBounds(mouseX, mouseY)) {
                    ...
                    WaypointData.INSTANCE.categories.remove(element.category);
                }
                element.click(mouseX, mouseY);
            }
        }
